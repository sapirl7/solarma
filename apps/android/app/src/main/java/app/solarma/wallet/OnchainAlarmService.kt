        ...
        suspend fun claimDeposit(
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    val txBytes =
                        transactionBuilder.buildClaimTransactionByPubkey(
                            owner = owner,
                            alarmPda = alarmPda,
                        )
        ...
        suspend fun snoozeAlarm(
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    val alarmPda = resolveAlarmPda(alarm, owner)
                    val txBytes =
                        transactionBuilder.buildSnoozeTransactionByPubkey(
                            owner = owner,
                            alarmPda = alarmPda,
                        ...
                        try {
                            ...
                            if (existing != null) {
                                ...
                                if (isResolved && existing.hasDeposit) {
                                    ...
                                    Log.i(
                                        TAG,
                                        "Sync: cleared stale deposit for " +
                                            "${account.pubkey} " +

...
class SolanaRpcClient
    ...
    constructor() {
        companion object {
            ...
            private val DEFAULT_DEVNET_ENDPOINTS =
                listOf(
            ...
            private val DEFAULT_MAINNET_ENDPOINTS =
                listOf(
         ...
         * Get account info.
         */
        suspend fun getAccountInfo(pubkey: String): Result<String> =
            withContext(Dispatchers.IO) {
                try {
                    val response =
                        makeRpcCallWithFallback(
         ...
         */
        suspend fun getSignatureStatus(signature: String): Result<SignatureStatus> =
            withContext(Dispatchers.IO) {
                try {
                    val response =
                        makeRpcCallWithFallback(
         ...
         */
        suspend fun accountExists(pubkey: String): Result<Boolean> =
            withContext(Dispatchers.IO) {
                try {
                    val response =
                        makeRpcCallWithFallback(
         ...
         */
        suspend fun sendTransactionFanOut(signedTx: ByteArray): Result<String> =
            withContext(Dispatchers.IO) {
                val base64Tx = android.util.Base64.encodeToString(signedTx, android.util.Base64.NO_WRAP)
                ...
                val healthyEndpoints =
                    currentEndpoints.filter { endpoint ->
                        val lastFailed = endpointLastFailedAt[endpoint]
                        val failures = endpointFailureCount[endpoint] ?: 0
                        val cooldown =
                            (BASE_BACKOFF_MS * (1L shl failures.coerceAtMost(6)))
         ...
         */
        private fun makeRpcCallWithFallback(
            method: String,
            params: String,
        ): String {
         ...
         */
        suspend fun checkTimeDrift(): Long? =
            withContext(Dispatchers.IO) {
                try {
                    // Get latest slot
                    val slotResponse =
                    ...
                    val slot =
                        parseResultValue(slotResponse) as? Number
                            ?: return@withContext null

                    // Get block time for that slot
                    val blockTimeResponse =
                        makeRpcCallWithFallback(
                            method = "getBlockTime",
                            params = "[${slot.toLong()}]",
                        )
                    val blockTimeUnix =
                        parseResultValue(blockTimeResponse) as? Number
                            ?: return@withContext null

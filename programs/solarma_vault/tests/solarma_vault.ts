...
describe("solarma_vault", () => {
    ...
    before(async function () {
        ...
        if (balance < 0.1 * LAMPORTS_PER_SOL) {
            ...
            try {
                const sig = await provider.connection.requestAirdrop(owner.publicKey, LAMPORTS_PER_SOL);
                await provider.connection.confirmTransaction(sig);
                console.log("Airdrop successful");
            } catch (e) {
                console.log("Airdrop failed (rate limited), tests may fail if insufficient funds");
            }
        }
    });
    ...
    describe("Initialize", () => {
        it("Creates a user profile", async () => {
            ...
            // Skip if profile already exists (devnet persistent state)
    ...
    describe("Snooze", () => {
        ...
        // Skip: Each snooze extends alarm_time by 5 minutes (DEFAULT_SNOOZE_EXTENSION_SECONDS=300).
        // Testing multiple snoozes on devnet would require waiting 5+ minutes between calls.
        // The exponential cost calculation logic is verified by unit tests and single-snooze test above.
        it.skip("Snooze cost doubles each time (requires 5-min wait)", async () => {
            ...
            // Wait for extended alarm_time to pass (snooze adds 5 minutes, but devnet time may drift)
            // In practice we wait 3s since snooze_extension is relative to current time
            await new Promise(resolve => setTimeout(resolve, 3000));
